...
function makeNewId() {
  if(todos.length > 0) { //객체가 하나라도 존재하니?
...
function makeNewToDoNode(newTodo) {
  ...
  */
  ...
  //클래스 이름을 두 개 add할 때는 매개값을 두개 보내야 한다.
  //한번에 공백 포함 두개 설정하면 에러다.
...
function insertToDoData() {
  ...
  todos.push(newTodo); //배열의 맨 마지막에 객체를 추가해줌  
...
function enterModifyMode($modSpan) {
  ...
  $modInput.setAttribute('type', 'text'); //생략 가능
...
(function() {
  ...
  $addBtn.addEventListener('click', e => {
    ...
    //form태그 안의 button은 type을 명시하지 않으면 자동 submit이 실행된다.
